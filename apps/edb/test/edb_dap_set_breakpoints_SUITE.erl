...
-export([test_set_breakpoint_by_compile_metadata_source/1]).
-export([test_set_breakpoint_by_module_name_without_compile_metadata_source/1]).

all() ->
    [
        test_error_set_breakpoint_bad_line,
        test_error_set_breakpoint_unknown_module,
        test_set_breakpoint_by_compile_metadata_source,
        test_set_breakpoint_by_module_name_without_compile_metadata_source
    ].
...
test_set_breakpoint_by_compile_metadata_source(Config) ->
    SourcePath = filename:join(proplists:get_value(priv_dir, Config), "breakpoint.source"),
    Module1 = edb_generic_source_breakpoint_1,
    Module2 = edb_generic_source_breakpoint_2,

    {ok, Client, #{peer := Peer, srcdir := SrcDir}} = edb_dap_test_support:start_session_via_launch(Config, #{}),
    ok = edb_dap_test_support:configure(Client, []),
    ok = load_module_object(Peer, SrcDir, Module1, module_object(Module1, SourcePath, 6)),
    ok = load_module_object(Peer, SrcDir, Module2, module_object(Module2, SourcePath, 8)),

    Response = edb_dap_test_client:set_breakpoints(Client, #{
        source => #{path => edb_test_support:safe_string_to_binary(SourcePath)},
        breakpoints => [#{line => 6}, #{line => 8}, #{line => 42}]
    }),
...
test_set_breakpoint_by_module_name_without_compile_metadata_source(Config) ->
